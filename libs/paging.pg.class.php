...
private $next_page_text = 'Следующая страница >';
private $prev_page_text = '< Предыдущая страница';
private $result_text_pattern = "<font size=\"-2\">Записи с %s по %s из %s</font>";
private $page_var = 'p';
...
public function __construct($db, $orderColumns, $page_size = 10, $q='', $page_var='p')
{
    $this->db = $db;
...
public function set_result_text_prefix ($prefix)
{
	$this->result_text_pattern = "<font size=\"-2\">".$prefix." с %s по %s из %s</font>";
}
...
public function get_page($q='')
{
	...
	$r = pg_query($this->db, $this->query_paging($this->q));
	#print $this->query_paging($this->q);
   
	
	$this->total_rows = pg_num_rows(pg_query($this->db, $this->q));
...
public function get_result_text()
{
    ...
    $start = ($this->total_rows > 0 ? $start : 0);
...
private function get_page_link($page, $text='')
{
    ...
    if ($page != $this->cur_page)
    {
        ...
        return '<a class="paging" id="paging" href="'.$url.'">'.$text.'</a>';
    }
...
private function query_paging()
{
    ...
    if ($this->page_size != 0)
    {
		...
		$q = preg_replace('/^SELECT\s+/i', '', $this->q);
		#$q = "SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY {$this->orderColumns}) AS rownum, ".$q. ") AS foo WHERE rownum > $start AND rownum <= ({$this->page_size}+$start)";
		#$q = preg_replace('/^SELECT\s+/i', 'SELECT SQL_CALC_FOUND_ROWS ', $this->q)." LIMIT {$this->page_size} offset {$start}";
		$q = $this->q." order by $this->orderColumns LIMIT {$this->page_size} offset {$start}";
	}
